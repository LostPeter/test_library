...
public:
 ...
 private:
    ...
    void initWindow() 
    {
        ...
        this->window = glfwCreateWindow(WIDTH, HEIGHT, "test_library_00002_assimp_001_mesh", nullptr, nullptr);
    ...
    void initVulkan()
    {
        ...
        //loadModel_Using_tinyobj();
        loadModel_Using_assimp();
        createVertexBuffer();
        createIndexBuffer();
    ...
    void createInstance()
    {
        ...
        appInfo.pApplicationName = "test_library_00002_assimp_001_mesh";
    ...
    void loadModel_Using_assimp()
    {
        ...
        const aiScene* pScene = importer.ReadFile(pathModel.c_str(), aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices);
        if (!pScene)
        {
            throw std::runtime_error(warn + err);
        }
        ...
        //2> Load Mesh
        unsigned int count_mesh = pScene->mNumMeshes;
        if (count_mesh > 0)
        {
            const aiMesh* paiMesh = pScene->mMeshes[0];

            const aiVector3D zero3D(0.0f, 0.0f, 0.0f);
            unsigned int count_vertices = paiMesh->mNumVertices;
            ...
            unsigned int count_faces = paiMesh->mNumFaces;
