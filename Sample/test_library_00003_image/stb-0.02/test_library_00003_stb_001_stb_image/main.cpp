...
int main()
{
    cout << "********** Test stb 001 stb_image ********** " << endl;
    {
        int texWidth = 0;
        int texHeight = 0;
        int texChannels = 0;
        std::string pathTexture = GetAssetsPath() + "Assets/Texture/texture.jpg";
        unsigned char *data = stbi_load(pathTexture.c_str(), 
                                        &texWidth, 
                                        &texHeight, 
                                        &texChannels, 
                                        STBI_rgb_alpha);
        if (data != nullptr)
        {
            std::cout << "Load texture.jpg success: Width: " << texWidth << ", Height: " << texHeight << ", Channels: " << texChannels << std::endl;
        }
        else
        {
            std::cout << "Load texture.jpg failed !" << std::endl;
        }
    }
    cout << "********** Test stb 001 stb_image ********** " << endl;
