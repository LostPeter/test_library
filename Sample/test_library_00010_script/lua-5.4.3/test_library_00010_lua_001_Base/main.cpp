...
int main()
{
    ...
    cout << "********** Test lua 001 Base ********** " << endl;
    {
        int error = luaL_loadfile(L, "../Assets/Lua/test_0001.lua") | lua_pcall(L, 0, 0, 0);
        if (error != 0)
        {
            cout << "Assets/Lua/test_0001.lua run error !" << endl;
        }

        const char luaScript[] = "function sum(a,b) return a+b; end";
        int load_state = luaL_loadbuffer(L, luaScript, strlen(luaScript), luaScript);
        lua_pcall(L, 0, 0, 0);
