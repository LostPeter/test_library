...
int main()
{
    cout << "********** Test sol2 001 Lua ********** " << endl;
    {
        sol::state luaState;

        int x = 0;
        cout << "Before AddSelf: x = " << x << endl;
        luaState.set_function("AddSelf", [&x]{ ++x; });
        luaState.script("AddSelf()");
        cout << "After AddSelf: x = " << x << endl;
        luaState.script("AddSelf()");
        cout << "After AddSelf: x = " << x << endl;

        luaState.new_usertype<Test>("Test", "boop", &Test::boop);
        luaState.script("test=Test.new() test.boop=1");
        cout << "boop = " << luaState.get<Test>("test").boop << endl;
    }
    cout << "********** Test sol2 001 Lua ********** " << endl;
