...
void test_rapidxml_parse()
{
	std::string pathXML = GetAssetsPath() + "Assets/Test/xml_in.xml";
	rapidxml::file<> doc(pathXML.c_str());
	if (doc.data() != nullptr)
	{
		cout << "test_rapidxml_parse: Load file: [" << pathXML << "] success !" << endl;
        cout << "Length: " << doc.size() << endl;
        cout << doc.data() << endl;
    }
	else
	{
		cout << "test_rapidxml_parse: Load file: [" << pathXML << "] failed !" << endl;
	}
}
...
int main()
{
    cout << "********** Test rapidxml 001 Parse ********** " << endl;
    {
        cout << "***** <1> test_rapidxml_parse ***** " << endl;
        {
            test_rapidxml_parse();
        }
        cout << "***** <1> test_rapidxml_parse ***** " << endl;
