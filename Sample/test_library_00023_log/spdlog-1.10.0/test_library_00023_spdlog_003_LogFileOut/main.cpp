...
int main()
{
    cout << "********** Test spd 001 Base ********** " << endl;
    {
        try 
        {
            auto logger = spdlog::basic_logger_mt("logger", "logs/log.txt");

            logger->info("Welcome to spdlog: [info], {}", 1);
            logger->warn("Welcome to spdlog: [warn], {:08d}", 12);
            logger->error("Welcome to spdlog: [error], {:03.2f}", 1.23456);
            logger->critical("Welcome to spdlog: [critical], {0:d};  hex: {0:x};  oct: {0:o}; bin: {0:b}", 42);
