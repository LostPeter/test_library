...
int main()
{
    cout << "********** Test spd 002 LogStdOut ********** " << endl;
    {
        auto console = spdlog::stdout_color_mt("console");
        auto err_logger = spdlog::stderr_color_mt("stderr");

        spdlog::get("console")->info("Welcome to spdlog: [info], {}", 1);
        spdlog::get("console")->warn("Welcome to spdlog: [warn], {:08d}", 12);
        spdlog::get("console")->error("Welcome to spdlog: [error], {:03.2f}", 1.23456);
        spdlog::get("console")->critical("Welcome to spdlog: [critical], {0:d};  hex: {0:x};  oct: {0:o}; bin: {0:b}", 42);
        spdlog::get("console")->info("Positional args are {1} {0}..", "too", "supported");
        spdlog::get("console")->info("{:<30}", "left aligned");
        spdlog::get("console")->set_level(spdlog::level::debug); // Set global log level to debug
        spdlog::get("console")->debug("This message should be displayed..");    
